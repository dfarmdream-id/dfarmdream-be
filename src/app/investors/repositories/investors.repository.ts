...
export class InvestorsRepository {
  ...
  public delete(where: Prisma.InvestorWhereUniqueInput) {
    return from(
      this.prismaService.investor.update({
        where,
  ...
  public firstOrThrow(
    where: Prisma.InvestorWhereUniqueInput,
    select?: Prisma.InvestorInclude,
  ) {
    return from(
      this.prismaService.investor.findUnique({ where, include: select }),
    ).pipe(
      catchError((error) => {
        throw error;
      }),
